...
static void form0_exterior_facet_integral_otherwise(double *__restrict__ A, double const *__restrict__ coords, double const *__restrict__ w_0, double const *__restrict__ w_1, double const *__restrict__ w_2, double const *__restrict__ w_3, uint32_t const *__restrict__ facet)
{
  ...
  for (int32_t ip = 0; ip <= 1; ++ip)
  {
    ...
    t19 = t10 * 0.40960000000000013 * t18 + t9 * 0.40960000000000013 * t17;
    t20 = t20 + (w_2[0] * (t19 > 0.0 ? t19 * w_1[0] : 0.0) + w_2[0] * (t19 < 0.0 ? t19 * w_3[0] : 0.0)) * t15[ip] * t14;
  }
  {
    int32_t const j = 0;
...
void wrap_form0_exterior_facet_integral_otherwise(int32_t const start, int32_t const end, double *__restrict__ dat1, double const *__restrict__ dat2, double const *__restrict__ dat3, double const *__restrict__ dat4, double const *__restrict__ glob0, double const *__restrict__ glob1, uint32_t const *__restrict__ dat0, int32_t const *__restrict__ map0, int32_t const *__restrict__ map1, int32_t const *__restrict__ map2)
{
  double t0[1];
  double t1[3 * 2];
  double t2[7 * 2];
  ...
  for (int32_t n = start; n <= -1 + end; ++n)
  {
    ...
    form0_exterior_facet_integral_otherwise(&(t0[0]), &(t1[0]), &(t2[0]), &(t3[0]), &(glob0[0]), &(glob1[0]), &(dat0[n]));
    {
      int32_t const i16 = 0;
