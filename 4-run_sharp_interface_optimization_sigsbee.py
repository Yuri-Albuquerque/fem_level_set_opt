...
model["mesh"] = {
    ...
    "meshfile": "meshes/sigsbee2b_guess3.msh",
    "initmodel": "velocity_models/sigsbee2b_guess3.hdf5",
    "truemodel": "velocity_models/sigsbee2b_true3.hdf5",
    "background": "velocity_models/background_3.hdf5",
}
model["PML"] = {
...
sources = spyro.create_transect((-0.01, 4.0), (-0.01, 27.0), 3)
model["acquisition"] = {
    "source_type": "Ricker",
...
def calculate_functional(model, mesh, comm, vp, sources, receivers, iter_num):
    ...
    for sn in range(model["acquisition"]["num_sources"]):
        if spyro.io.is_owner(comm, sn):
            ...
            f = "shots/sigsbee2b_true_" + str(sn) + ".dat"
...
def optimization(
    ...
    while iter_num < max_iter:
        # creat a new weighting function
        if iter_num > 100:
            factor = 10
            beta0 = beta0_init = 1.5*factor
            weighting = create_weighting_function(
...
vp = optimization(
    model, mesh, V, comm, vp, vp_background, sources, receivers, max_iter=50
)
