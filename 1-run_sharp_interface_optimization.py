...
def create_weighting_function(V, const=100.0, M=5, width=0.1, show=False):
    ...
    Z, X = coords.dat.data_ro_with_halos[:,
                                         0], coords.dat.data_ro_with_halos[:, 1]
...
def oneDiskOptimization():
    ...
    recvs = spyro.create_transect((-0.01, 0.01), (-0.01, 0.99), num_receivers)
    sources = spyro.create_transect((-0.01, 0.01), (-0.01, 0.99), num_sources)
    ...
    if comm.ensemble_comm.rank == 0:
        evolution_of_velocity = File(
            "../../results/evolution_of_velocity.pvd", comm=comm.comm)
    ...
    vp = optimization(model, mesh, V, comm, vp,
