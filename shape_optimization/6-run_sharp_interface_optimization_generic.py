...
model["mesh"] = {
    ...
    "meshfile": "meshes/evolution_of_velocity_62.msh",
    "initmodel": "velocity_models/vp_new_62.hdf5",
...
def create_weighting_function(V, const=100.0, M=5, width=0.1, show=False):
    ...
    w = const * (
        ...
        + np.maximum(
            ...
            h(b0 - Z, cz, wtop) + 0.0*h(Z - b1, cz, wbottom),
...
def calculate_functional(model, mesh, comm, vp, sources, receivers, iter_num):
    ...
    for sn in range(model["acquisition"]["num_sources"]):
        ...
        if spyro.io.is_owner(comm, sn):
            ...
            if comm.comm.rank == 0 and plt_comparison:
    ...
    if comm.ensemble_comm.rank == 0:
        ...
        with open("./results/cost_history.txt", "r") as ch:
            if iter_num % 10 == 0:
                lines = ch.readlines()
...
def optimization(
    ...
    while iter_num < max_iter:
        ...
        # write ALL velocity updates to a vtu file
        if comm.ensemble_comm.rank == 0:
        ...
        ###################################
        ## Check point for remesh proces  #
        ###################################
        # with CheckpointFile("./results/evolution/vp_new_"+str(iter_num)+".hdf5", 'w') as afile:
        #     afile.save_mesh(mesh)  # optional
        #     afile.save_function(vp_new)
        ...
        if J_new < J_old:
            ...
            ###################################
            #### begining of re-mesh process  ####
            ###################################
            if iter_num > 0 and iter_num % 1 == 0:
                print(f'iteration number: {iter_num}')
                _, _, data_new = spyro.io.write_function_to_grid(vp_new, V, 0.001)
                # plt.imshow(data_new)
                # plt.colorbar()
                # plt.show()
